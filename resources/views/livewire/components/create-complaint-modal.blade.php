<div>
    ...
    @if ($isOpen)
        <div x-data x-show="true" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
            ...
            @keydown.escape.window="$wire.closeModal()" class="fixed inset-0 z-[999] bg-black bg-opacity-45"
            style="background-color: rgba(0, 0, 0, 0.45);"></div>
    @endif

    {{-- Modal Container --}}
    @if ($isOpen)
        <div x-data x-show="true" x-transition:enter="ease-out duration-300"
            ...
            ">
                ...
                <div class="relative mb-4">
                    <h2 class="text-xl font-bold text-center text-[#212121] mb-4"
                        style="font-family: 'Inter', sans-serif; font-size: 20px; font-weight: 700;">
                        Buat Pengaduan Baru
                    </h2>

                    {{-- Close Button --}}
                    <button type="button" wire:click="closeModal" aria-label="Tutup modal" title="Tutup"
                    ...
                    ">
                        <svg class="w-4 h-4 text-[#D32F2F] group-hover:text-[#C62828] transition-colors" fill="none"
                ...
                <div class="mb-5 rounded-lg border"
                    style="background-color: #E3F2FD; border: 1px solid #BBDEFB; border-radius: 8px; padding: 12px 16px;">
                ...
                <form wire:submit.prevent="submit" class="flex flex-col" style="gap: 20px;">
                    {{-- Judul Pengaduan Field --}}
                    <div>
                        <label class="block text-sm font-medium text-[#212121] mb-2">
                            Judul Pengaduan
                        </label>
                        <input type="text" wire:model="judul" placeholder="Masukkan judul pengaduan"
                    ...
                    <div>
                        ...
                        <textarea wire:model.defer="lokasi" rows="3"
                            placeholder="Tuliskan lokasi kejadian, contoh: Pasar Raya Padang, dekat Masjid Nurul Iman"
                            class="w-full px-3 py-2 border border-[#E0E0E0] rounded-md
               ...
               focus:border-transparent text-sm resize-none"
                    ...
                    <div>
                        ...
                        <textarea wire:model="deskripsi" rows="4" placeholder="Jelaskan detail pengaduan anda …"
                    ...
                    <div>
                        ...
                        <div wire:click="$refs.fileInput.click()" x-data="{ isDragging: false }"
                            ...
                            gap: 8px;
                        ">
                            <input type="file" wire:model="foto_bukti" x-ref="fileInput" class="hidden"
                                accept="image/*">

                            @if ($foto_bukti)
                                <div class="w-full h-full flex flex-col items-center justify-center">
                                    <img src="{{ $foto_bukti->temporaryUrl() }}" alt="Preview"
                                        class="max-w-full max-h-32 object-contain rounded">
                                    <span
                                        class="text-xs text-[#757575] mt-2">{{ $foto_bukti->getClientOriginalName() }}</span>
                                </div>
                            @else
                                <svg class="w-12 h-12 text-[#90CAF9]" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
